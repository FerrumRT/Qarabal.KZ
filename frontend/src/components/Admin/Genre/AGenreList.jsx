...
import AddGenreModal from './AAddGenreModal';
...
export default function GenreList(){
  ...
  const pageCount = () => {
    let c;
    c = Math.floor(reducers.genre.list.length / 5);
    if(reducers.genre.list.length % 5 != 0) c++;
    return c;
  }
    ...
    <div style={{"backgroundColor":"rgba(0, 0, 0, 0.5)"}} className="p-4">
        <div className="row">
          ...
          <div className="col-8">
            ...
            <h3 className="float-left">Genres</h3>
            <AddGenreModal/>
            <table className="table table-dark">
              ...
              <tbody>
                {reducers.genre.list.slice((page-1)*5, page*5).map(genre=>{
                  return(
                    <tr key={genre.id}>
                      <th scope="row">{genre.id}</th>
                      <th>{genre.genre}</th>
                      <th><Link to={`/admin/genre/${genre.id}`} className="btn btn-light">Details</Link></th>
                    </tr>
                  )
                })}
              </tbody>
